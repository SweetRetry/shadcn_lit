...
export const createWalletBalanceTemplate = (wallet?: WalletInfo) => {
  return html`<section
    class="flex justify-between rounded border border-border p-4"
  >
    <div class="space-y-2">
...
export class ExWallet extends TailwindElement {
  ...
  private renderCryptos(wallet?: WalletInfo) {
    if (wallet?.assets) {
      ...
      return html` <section class="border border-border p-4 rounded mt-4">
  ...
  render() {
    ...
    return html` <section class="h-full rounded">
      <ex-spinner .loading=${this.loading}>
        ${createWalletBalanceTemplate(wallet)} ${this.renderCryptos(wallet)}
      </ex-spinner>
    </section>`;
  }
}
